...
export default class ErrorHandler {
  ...
  badRequest(err?: string) {
    const error = new ErrorGeneric(
      `❌ Bad Request: ${this.useTemplate(err)}`,
      400
    );
  ...
  unauthorized(err?: string) {
    const error = new ErrorGeneric(
      `❌ Unauthorized: ${this.useTemplate(err)}`,
      401
    );
    errLogger.error(error.errMsg);
    return error;
  ...
  forbidden(err?: string) {
    const error = new ErrorGeneric(
      `❌ Forbidden: ${this.useTemplate(err)}`,
      403
    );
    errLogger.error(error.errMsg);
    return error;
  ...
  notFound(err?: string) {
    const error = new ErrorGeneric(
      `❌ Not Found: ${this.useTemplate(err)}`,
      404
    );
    errLogger.error(error.errMsg);
    return error;
  ...
  internal(err?: string) {
    const error = new ErrorGeneric(
      `❌ Server Internal Error: ${this.useTemplate(err)}`,
      500
    );
    errLogger.error(error.errMsg);
    return error;
