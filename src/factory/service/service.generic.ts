...
export default class ServiceGeneric<T> implements IServiceGeneric<T> {
  ...
  public async update(id: string, data: Partial<T>) {
    let updated = (await this.db
      .change(`${this.dbtable}:${id}`, data as Record<string, unknown>)
      .catch((e) => {
        const err = this.errorHandler(e, 500);
        return err;
      })) as Result<T> | ErrorGeneric;
  ...
  public async isThere(id: string) {
    const dbres = (
      await this.db
        .query(`select * from ${this.dbtable} where id = ${this.dbtable}:${id}`)
        .catch((e) => {
          const err = this.errorHandler(e, 500);
          return [err];
        })
  ...
  public async find(data: Omit<Partial<T>, "id">) {
    ...
    const dbres = (
      await this.db
        .query<T>(`select * from ${this.dbtable} where ${query}`)
        .catch((e) => {
          const err = this.errorHandler(e, 500);
          return [err];
        })
